...
enum class MalformedResponseMode : uint8_t {
  NONE = 0,
  WRONG_BYTE_COUNT = 1,
  INCOMPLETE_RESPONSE = 2
};
enum class UartFaultMode : uint8_t { NONE = 0, PARITY = 1, FRAMING = 2 };
...
public:
  ...
  void set_parent(QuattOduSimulator *parent) {
    this->parent_simulator_ = parent;
  }
  void set_response_delay_ms(uint16_t value) {
    this->response_delay_ms_ = value;
  }
  ...
  void set_exception_injection(bool enabled, uint16_t start_address,
                               uint8_t exception_code) {
    this->exception_enabled_ = enabled;
    this->exception_start_address_ = start_address;
    this->exception_code_ = exception_code;
  }
  void set_malformed_injection(MalformedResponseMode mode,
  ...
  uint32_t uart_fault_restore_error_count() const {
    return this->uart_fault_restore_error_count_;
  }
  bool uart_fault_active() const { return this->uart_fault_active_; }
...
protected:
  ...
  void process_request_(uint8_t address, std::span<const uint8_t> pdu,
                        bool respond);
  void process_read_(uint8_t address, uint8_t function_code,
                     uint16_t start_address, uint16_t count, uint8_t odu_index);
  void process_write_single_(uint8_t address, std::span<const uint8_t> pdu,
                             uint16_t start_address, uint8_t odu_index,
                             bool respond);
  void process_write_multiple_(uint8_t address, std::span<const uint8_t> pdu,
                               uint16_t start_address, uint16_t count,
                               uint8_t odu_index, bool respond);
  void send_exception_(uint8_t address, uint8_t function_code,
                       modbus::ExceptionCode exception);
  bool send_pdu_(uint8_t address, const uint8_t *pdu, uint16_t pdu_size);
  void pump_tx_();
  void request_uart_fault_(UartFaultMode mode);
  void pump_uart_fault_();
  bool bus_idle_for_uart_fault_();
  bool restore_uart_after_fault_();
  ...
  uint8_t exception_code_{
      static_cast<uint8_t>(modbus::ExceptionCode::SERVICE_DEVICE_FAILURE)};
...
public:
  ...
  QuattOduSimulatorModel &model(uint8_t index) {
    return this->models_[index < 2U ? index : 0U];
  }
  const QuattOduSimulatorModel &model(uint8_t index) const {
    return this->models_[index < 2U ? index : 0U];
  }

  void set_profile(uint8_t index, uint8_t profile) {
    if (index >= this->models_.size())
      return;
    if (profile > static_cast<uint8_t>(Profile::V2_NEW))
      profile = static_cast<uint8_t>(Profile::DISABLED);
    this->models_[index].configure(static_cast<Profile>(profile));
  }
  uint8_t get_profile(uint8_t index) const {
    return static_cast<uint8_t>(this->model(index).state().profile);
  }

  void set_address(uint8_t index, uint8_t address) {
    if (index >= this->addresses_.size() || address == 0U || address > 247U)
      return;
    const uint8_t other = index == 0U ? 1U : 0U;
    if (this->addresses_[other] == address)
      return;
    this->addresses_[index] = address;
  }
  uint8_t get_address(uint8_t index) const {
    return this->addresses_[index < 2U ? index : 0U];
  }
  int8_t find_odu_by_address(uint8_t address) const {
    for (uint8_t index = 0; index < this->addresses_.size(); index++)
      if (this->addresses_[index] == address)
        return static_cast<int8_t>(index);
    return -1;
  }

  void set_simulation_enabled(bool enabled) {
    this->simulation_enabled_ = enabled;
  }
  void set_responses_enabled(bool enabled) {
    this->responses_enabled_ = enabled;
  }
  void set_odu_responses_enabled(uint8_t index, bool enabled) {
    if (index < this->odu_responses_enabled_.size())
      this->odu_responses_enabled_[index] = enabled;
  }
  ...
  bool should_respond(uint8_t index) const {
    return index < this->models_.size() && this->simulation_enabled_ &&
           this->responses_enabled_ && this->odu_responses_enabled_[index] &&
           this->models_[index].enabled();
  }
  ...
  void set_defrost(uint8_t index, bool value) {
    this->model(index).set_defrost(value);
  }
  void set_fault_word(uint8_t index, uint8_t word, uint16_t value) {
    if (word < 3U)
      this->model(index).mutable_state().fault_words[word] = value;
  }
  ...
  void set_force_no_flow(uint8_t index, bool value) {
    this->model(index).mutable_state().force_no_flow = value;
  }
  ...
  bool apply_runtime_modified_preset(uint8_t index) {
    return this->model(index).apply_runtime_modified_preset();
  }
  void reset_diagnostics() {
