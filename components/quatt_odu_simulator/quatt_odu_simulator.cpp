...
void QuattOduModbusHub::loop() {
  this->pump_tx_();
  this->pump_uart_fault_();
  if (this->tx_active_ || this->uart_fault_active_)
    return;
  modbus::Modbus::loop();
  if (!this->pending_.active ||
      static_cast<int32_t>(millis() - this->pending_.due_ms) < 0) {
    this->pump_uart_fault_();
    return;
  }
  if (this->tx_active_ || this->uart_fault_active_ || this->available() ||
  ...
  this->process_request_(address,
                         std::span<const uint8_t>(pdu.data(), pdu_size), true);
}
...
void QuattOduModbusHub::on_shutdown() {
  ...
  this->active_uart_fault_ = UartFaultMode::NONE;
  if (this->flow_control_pin_ != nullptr)
    this->flow_control_pin_->digital_write(false);
...
void QuattOduModbusHub::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  UART fault injection: %s",
                YESNO(this->uart_fault_injection_enabled_));
  LOG_PIN("  Flow Control Pin: ", this->flow_control_pin_);
}

void QuattOduModbusHub::request_uart_parity_fault() {
  this->request_uart_fault_(UartFaultMode::PARITY);
}

void QuattOduModbusHub::request_uart_framing_fault() {
...
void QuattOduModbusHub::request_uart_fault_(UartFaultMode mode) {
  ...
  this->pending_uart_fault_ = mode;
}

bool QuattOduModbusHub::bus_idle_for_uart_fault_() {
  return !this->tx_active_ && !this->pending_.active && !this->available() &&
         this->rx_buffer_.empty() && this->tx_delay_remaining() == 0;
}

bool QuattOduModbusHub::restore_uart_after_fault_() {
  if (this->parent_ == nullptr)
    return false;
  auto *idf_uart = static_cast<uart::IDFUARTComponent *>(this->parent_);
  const auto uart_num =
      static_cast<uart_port_t>(idf_uart->get_hw_serial_number());
  const esp_err_t status = uart_set_parity(uart_num, UART_PARITY_EVEN);
  if (status != ESP_OK) {
    this->uart_fault_restore_error_count_++;
    ESP_LOGE(TAG, "Failed to restore M2 UART parity to even: %s",
...
void QuattOduModbusHub::pump_uart_fault_() {
  if (this->parent_ == nullptr)
    return;
  auto *idf_uart = static_cast<uart::IDFUARTComponent *>(this->parent_);
  const auto uart_num =
  ...
  if (this->uart_fault_active_) {
    ...
    if (status == ESP_OK ||
        static_cast<int32_t>(millis() - this->uart_fault_deadline_ms_) >= 0) {
      if (status != ESP_OK)
        ESP_LOGW(TAG,
                 "M2 UART fault transmission did not drain before deadline");
      this->restore_uart_after_fault_();
      if (this->flow_control_pin_ != nullptr)
        this->flow_control_pin_->digital_write(false);
      this->uart_fault_active_ = false;
      ...
      ESP_LOGE(TAG, "Failed while waiting for M2 UART fault transmission: %s",
               esp_err_to_name(status));
      this->restore_uart_after_fault_();
      if (this->flow_control_pin_ != nullptr)
        this->flow_control_pin_->digital_write(false);
  ...
  if (this->pending_uart_fault_ == UartFaultMode::NONE ||
      !this->uart_fault_injection_enabled_ || !this->bus_idle_for_uart_fault_())
    return;

  const UartFaultMode mode = this->pending_uart_fault_;
  this->pending_uart_fault_ = UartFaultMode::NONE;
  if (this->flow_control_pin_ != nullptr)
    this->flow_control_pin_->digital_write(true);

  static constexpr uint8_t FAULT_BYTE = 0x55U;
  int written = -1;
  if (mode == UartFaultMode::PARITY) {
    const esp_err_t status = uart_set_parity(uart_num, UART_PARITY_ODD);
    if (status == ESP_OK)
      written = uart_tx_chars(uart_num,
                              reinterpret_cast<const char *>(&FAULT_BYTE), 1U);
    else
      ESP_LOGE(TAG, "Failed to configure M2 UART odd parity: %s",
               esp_err_to_name(status));
  } else if (mode == UartFaultMode::FRAMING) {
    written = uart_write_bytes_with_break(
        uart_num, reinterpret_cast<const char *>(&FAULT_BYTE), 1U, 20);
  }

  if (written != 1) {
    ESP_LOGE(TAG, "Failed to queue M2 UART %s fault",
             mode == UartFaultMode::PARITY ? "parity" : "framing");
    this->restore_uart_after_fault_();
    if (this->flow_control_pin_ != nullptr)
      this->flow_control_pin_->digital_write(false);
    this->uart_fault_rejected_count_++;
  ...
  this->active_uart_fault_ = mode;
  this->uart_fault_active_ = true;
  this->uart_fault_deadline_ms_ = millis() + 50U;
  if (mode == UartFaultMode::PARITY)
    this->uart_parity_fault_count_++;
  else
    this->uart_framing_fault_count_++;
}

void QuattOduModbusHub::parse_modbus_frames() {
  while (!this->rx_buffer_.empty()) {
    const size_t available = this->rx_buffer_.size();
    uint16_t frame_length = modbus::helpers::client_frame_length(
        this->rx_buffer_.data(), available);
    ...
    if (!modbus::helpers::is_client_pdu_standard(pdu.data(), pdu_size)) {
      if (address != modbus::BROADCAST_ADDRESS)
        this->send_exception_(address, function_code,
                              modbus::ExceptionCode::ILLEGAL_DATA_VALUE);
      continue;
    }
    this->queue_request_(address,
                         std::span<const uint8_t>(pdu.data(), pdu_size));
  }
  if (this->timeout_())
    this->clear_rx_buffer_(LOG_STR("timeout after partial request"), true);
}

void QuattOduModbusHub::queue_request_(uint8_t address,
  ...
  if (this->pending_.active) {
    const int8_t index =
        this->parent_simulator_ == nullptr
            ? -1
            : this->parent_simulator_->find_odu_by_address(address);
    if (index >= 0)
      this->record_drop_(static_cast<uint8_t>(index));
    return;
  }
...
void QuattOduModbusHub::process_request_(uint8_t address,
                                         std::span<const uint8_t> pdu,
                                         bool respond) {
  ...
  if (!respond) {
    for (uint8_t index = 0U; index < 2U; index++) {
      ...
      if (function_code == 0x06U)
        ...
        this->process_write_multiple_(address, pdu, start_address, count, index,
                                      false);
    }
    return;
  }
  ...
  if (!this->parent_simulator_->should_respond(index) ||
      (this->drop_every_nth_ > 0U &&
       this->request_sequence_ % this->drop_every_nth_ == 0U) ||
      (this->timeout_enabled_ &&
       start_address == this->timeout_start_address_)) {
  ...
  if (this->exception_enabled_ &&
      start_address == this->exception_start_address_) {
    model.mutable_state().protocol.exception_count++;
    this->send_exception_(
        address, function_code,
        static_cast<modbus::ExceptionCode>(this->exception_code_));
    return;
  }
...
void QuattOduModbusHub::process_read_(uint8_t address, uint8_t function_code,
  ...
  if (start_address <= 3069U &&
      static_cast<uint32_t>(start_address) + count > 3050U &&
  ...
  for (uint32_t register_address = start_address;
       register_address < static_cast<uint32_t>(start_address) + count;
       register_address++) {
  ...
  for (uint32_t register_address = start_address;
    ...
    if (!model.read_register(static_cast<uint16_t>(register_address), value)) {
      diagnostics.exception_count++;
      this->send_exception_(address, function_code,
                            modbus::ExceptionCode::SERVICE_DEVICE_FAILURE);
      return;
    }
  ...
  if (start_address == this->malformed_start_address_) {
    ...
    else if (this->malformed_mode_ ==
                 MalformedResponseMode::INCOMPLETE_RESPONSE &&
             response_size > 2U)
      response_size = static_cast<uint16_t>(
          response_size - std::min<uint16_t>(2U, response_size - 2U));
  }
  this->send_pdu_(address, response.data(), response_size);
}

void QuattOduModbusHub::process_write_single_(uint8_t address,
                                              std::span<const uint8_t> pdu,
  ...
  if (!model.can_write_register(start_address, value)) {
    ...
    if (respond)
      this->send_exception_(address, pdu[0],
                            modbus::ExceptionCode::ILLEGAL_DATA_VALUE);
    return;
  }
  model.write_register(start_address, value, millis());
  if (respond)
    this->send_pdu_(address, pdu.data(), static_cast<uint16_t>(pdu.size()));
}

void QuattOduModbusHub::process_write_multiple_(
    uint8_t address, std::span<const uint8_t> pdu, uint16_t start_address,
    uint16_t count, uint8_t odu_index, bool respond) {
  auto &model = this->parent_simulator_->model(odu_index);
  if (count == 0U || count > modbus::MAX_NUM_OF_REGISTERS_TO_WRITE ||
      pdu.size() != 6U + count * 2U || pdu[5] != count * 2U ||
      !modbus::helpers::address_range_fits(start_address, count)) {
  ...
  for (uint16_t offset = 0U; offset < count; offset++) {
    const uint16_t value =
        modbus::helpers::get_data<uint16_t>(pdu.data(), 6U + offset * 2U);
    if (!model.can_write_register(static_cast<uint16_t>(start_address + offset),
                                  value)) {
      model.record_rejected_write(static_cast<uint16_t>(start_address + offset),
                                  value, millis());
      if (respond)
        this->send_exception_(address, pdu[0],
                              modbus::ExceptionCode::ILLEGAL_DATA_VALUE);
  ...
  for (uint16_t offset = 0U; offset < count; offset++) {
    const uint16_t value =
        modbus::helpers::get_data<uint16_t>(pdu.data(), 6U + offset * 2U);
    model.write_register(static_cast<uint16_t>(start_address + offset), value,
                         millis());
  }
  if (respond) {
    uint8_t response[5] = {pdu[0], static_cast<uint8_t>(start_address >> 8U),
                           static_cast<uint8_t>(start_address & 0xFFU),
...
void QuattOduModbusHub::send_exception_(uint8_t address, uint8_t function_code,
                                        modbus::ExceptionCode exception) {
  const uint8_t response[2] = {
      static_cast<uint8_t>(function_code |
                           modbus::FUNCTION_CODE_EXCEPTION_MASK),
      static_cast<uint8_t>(exception)};
  this->send_pdu_(address, response, sizeof(response));
}

bool QuattOduModbusHub::send_pdu_(uint8_t address, const uint8_t *pdu,
                                  uint16_t pdu_size) {
  if (pdu_size == 0U || pdu_size > modbus::MAX_PDU_SIZE || this->tx_active_ ||
      this->available() || !this->rx_buffer_.empty() ||
      this->tx_delay_remaining() > 0)
    return false;
  ...
  const uint32_t transmit_us =
      (static_cast<uint32_t>(frame_size) * 11U * 1000000U +
       this->parent_->get_baud_rate() - 1U) /
...
void QuattOduModbusHub::record_drop_(uint8_t odu_index) {
  this->parent_simulator_->model(odu_index)
      .mutable_state()
      .protocol.dropped_response_count++;
}

void QuattOduSimulator::setup() {
  for (auto &model : this->models_)
    model.configure(Profile::DISABLED);
  this->last_update_ms_ = millis();
}

void QuattOduSimulator::update() {
  const uint32_t now = millis();
  const float dt_s = this->last_update_ms_ == 0U
                         ? this->get_update_interval() / 1000.0f
                         : (now - this->last_update_ms_) / 1000.0f;
...
void QuattOduSimulator::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Simulation enabled: %s",
                YESNO(this->simulation_enabled_));
  ESP_LOGCONFIG(TAG, "  Responses enabled: %s",
                YESNO(this->responses_enabled_));
  for (size_t index = 0; index < this->models_.size(); index++) {
    ESP_LOGCONFIG(
        TAG, "  ODU %u: address %u, profile %s",
        static_cast<unsigned>(index + 1U), this->addresses_[index],
